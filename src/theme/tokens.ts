...
//               （印章那一下「轻微回弹」由**插值停点**表达：缩放到 1.08 再收到 0.98 才停，
//                见 DoneView 的 STOPS。曲线本身仍不许过冲 —— 曲线一过冲，全项目所有用它
//                的地方都会跟着弹，那是另一回事。）
...
export const MOTION = {
  ...
  // 它不是一个独立动作，是 `seal` 的**后果** —— 故起点由 impact 决定（见 DoneView），
  // 时长只描述这一下抖动的余韵。别再长：超过 220ms 就从「纸抖」变成「屏在晃」。
  shock: 220,
} as const;

// 落印的冲击点：`stamp` 进度到这个比例时印章触纸，微震从此处起算。
// 单列一个常量而不是写死在插值里 —— 回弹曲线的形状（落 72%、回弹到 88%、定住）
// 与「微震从哪一刻开始」是同一件事的两个读法，改一个必须改另一个。
export const SEAL_IMPACT = 0.72;

// 打卡印记（P2.2「打卡日历印章化」）：一个打卡日 = 一枚朱印，印面边长按当日学习量四档递增。
//
// 为什么浓度用**印面大小**而不是朱砂深浅：先试过「浅朱 → 浓朱」的明度梯度（第一版就是），
// 但实测浅朱压在纸底上只有 1.4–2.7:1 —— 而印面只有几 dp，一个对比度不足 3:1 的小点
// 在屏幕上不构成「这天打过卡」的陈述（同一类错误的另一面见 §9.11②「中性不等于看不见」）。
// 同一支朱砂、盖得越重印面越大，既不牺牲对比度，也仍然是「浓度」这件事的直读。
// slot 是印记槽的高度：印记有无都占住这个高度，日历的行高才不会随打卡情况跳动。
//
// base 从 4 提到 6（2026-09-18 对照稿复核）：4–8.2dp 在 1:1 屏幕上只是个色点，
// 图例里四枚样本更是连「少→多」的递增都读不出来 —— 一个看不出差别的图例等于没有图例。
// 6–10.8dp 仍稳稳落在日期格内（格宽 48dp），而递增一眼可辨。
export const STAMP = { base: 6, step: 1.6, slot: 10 } as const;

/** 印记边长（dp）：档位 1..4 → 6 / 7.6 / 9.2 / 10.8。 */
export function stampSize(level: number): number {
  const lv = Math.max(1, Math.min(4, Math.round(level)));
  return STAMP.base + (lv - 1) * STAMP.step;
}
