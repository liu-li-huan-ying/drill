...
> 开发中，当前 **M1（数据管线 v1）+ UI 原型（v2.1）+ M2（最小闭环）+ M3（全量词库 v1）+ M4（检索/详情/设置/导入/词汇量测试）+ M5（统计看板 + 备份导出导入）+ M6（词根词缀拆解 + 手写助记与检索）+ 设计体系 v3.4（原型全量落地到实际界面）已完成**：
...
> - **设计体系 v3.4 · 原型全量落地到 App（本轮）**：不再是「原型好看」，**每个界面都按新稿改完了实际代码**。
